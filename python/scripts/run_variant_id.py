...
# Load amplicount outputs (configuration, coverage and variants) into pandas dataframe for analysis and ploting
...
# List of amplicons found - naming convention: name should start with 'chr'
...
# Variant Read Coverage, pairwise alignment to get variant classification
...
def get_variant_classification(contig, start, ref, alt, genome=ensembl_grch38):
    ...
    if len(ref) > len(alt):
        return 'Deletion', top_effect_name
    elif len(ref) < len(alt):
        return 'Insertion', top_effect_name
    else:
        return 'Mismatch', top_effect_name
        ...
        for seq in pivot_df_variants_filtered.columns.tolist():
            if not seq == 'barcode':
                if seq == 'other':
                    ...
                    i += 1
                    alignments = pairwise2.align.globalms(ref_sequence, seq, 5, -4, -3, -0.1)
                    ibase_start = 0
                    ibase_stop = 0
                    top_effects = set()
                    ...
                    for ibase in range(1, len(alignments[0][0])-1):
                        ...
                        if not alignments[0][0][ibase] == alignments[0][1][ibase]:
                            if not alignments[0][0][ibase] == '-' and not alignments[0][1][ibase] == '-':
                                ...
                                top_effect_type, top_effect_name = get_variant_classification(contig, start, ref, alt)
                                top_effect_type = 'Mismatch'
                        # looking at insertion and deletion
