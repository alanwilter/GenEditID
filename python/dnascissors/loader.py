...
class VariantLoader(Loader):
    ...
    def calculate_and_load_score(self):
        """
        filtering step:
        ---------------
        ...
        """
        results = self.session.query(VariantResult)\
        ...
        for variant in results:
            self.log.info('----------')
            self.log.info('variant: {} {}'.format(variant.chromosome, variant.position))
            sequencing_library_content = variant.sequencing_library_content
            ...
            if len(well.well_content.guides) == 1:
                ...
                for amplicon_selection in guide.amplicon_selections:
                    if variant.chromosome.endswith(amplicon_selection.amplicon.chromosome) and variant.position >= amplicon_selection.amplicon.start and variant.position <= amplicon_selection.amplicon.end:
                        ...
                        distance = abs(int(variant.position) - int(amplicon_selection.guide_location))
                ...
                if matched_amplicon_selection:
                    ...
                    variant.cut_site_on_target = matched_amplicon_selection.is_on_target
                    variant.cut_site_position = matched_amplicon_selection.guide_location
                    variant.distance = distance
                    variant.ge_score = score
                    self.log.info('guide: {}, onTarget: {}, cutSite: {}, distance: {}, score: {}'.format(guide.name, matched_amplicon_selection.is_on_target, matched_amplicon_selection.guide_location, distance, score))
                else:
                    self.log.info('guide: {}, not match found for this variant (position outside start/end of all amplicons), no score calculated'.format(guide.name))
                ...
                self.log.info('No guide found, no score calculated')
        self.session.commit()

    def load(self):
        self.load_sheet('SNVs', 'SNV')
        self.load_sheet('Indels', 'INDEL')
        self.calculate_and_load_score()
