...
# connect to tables
project <- tbl(db, "project") %>% rename(project_id=id, project_geid=geid)
...
protein_abundance_data <- inner_join(well, abundance, by="well_id") %>%
  ...
  select(target_name, guide_name, position, intensity_channel_700, intensity_channel_800) %>% 
  rename(Target=target_name, Guide=guide_name, Well=position, Channel700=intensity_channel_700, Channel800=intensity_channel_800)
